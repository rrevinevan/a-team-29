...
public class GUI extends Application {
  ...
  public void start(Stage primaryStage) throws Exception {
            ...
            if (reportType.getValue().contentEquals("Farm Report")) {
              ...
              try {
                ArrayList<String> farmIDList =
                    h.getAllFarmIDsInYear(Integer.parseInt(yearCombo.getValue()));
    ...
    Button printButt = new Button("Print Graph");
      ...
      if (reportType.getValue() != null) {
        ...
        if (reportType.getValue().equals("Farm Report")) {
          if (yearCombo.getValue() != null && farmCombo.getValue() != null
            ...
            stats.getChildren().add(printButt);
        ...
        if (reportType.getValue().equals("Annual Report")) {
          ...
          if (yearCombo.getValue() != null && orderCombo.getValue() != null
            ...
            // //////////////////////////////////////////////////////////////////////////////////
            else if (orderCombo.getValue().equals("Weight Decending")) {
              Collections.sort(dataSeries2.getData(), new Comparator<XYChart.Data>() {
                @Override
                public int compare(Data o1, Data o2) {
                  Number xValue1 = (Number) o1.getYValue();
            ...
            stats.getChildren().add(new Label("Average: "));
            double numFarmsInYear =
                (double) h.getAllFarmIDsInYear(Integer.parseInt(yearCombo.getValue())).size();
            double avrg =
                (double) (h.getTotalWeightOfAllFarmsInYear(Integer.parseInt(yearCombo.getValue())))
                    / numFarmsInYear;
        ...
        if (reportType.getValue().equals("Monthly Report")) {
          if (yearCombo.getValue() != null && monthCombo.getValue() != null
            ...
            // y-Axis Percentage
              ...
              for (int i = 0; i < allFarmsInMonth.size(); i++) {
                double currWeight =
                    (double) h.getTotalWeightOfFarmInMonth(allFarmsInMonth.get(i), month, year);
            ...
            stats.getChildren().add(new Label("Min:"));
            stats.getChildren()
                .add(new Label("" + h.getMinWeightInMonthForAll(year, month) + " lbs"));
            stats.getChildren().add(new Label("Max:"));
            stats.getChildren()
                .add(new Label("" + h.getMaxWeightInMonthForAll(year, month) + " lbs"));
        ...
        if (reportType.getValue().equals("Date Range Report")) {
          if (yearCombo.getValue() != null && monthCombo.getValue() != null
            ...
            if (h.getNumDaysInMonth(Integer.parseInt(yearCombo.getValue()),
              ...
              errorLabel.setTextFill(Color.web("990000"));
              errorLabel.setText("The selected starting day is out of the starting months boundry");
            } else if (Integer.parseInt(monthCombo2.getValue()) < Integer
                .parseInt(monthCombo.getValue())) {
              errorLabel.setTextFill(Color.web("990000"));
              errorLabel.setText("The ending month happens before the starting month");
            } else if ((Integer.parseInt(monthCombo2.getValue()) == Integer
                .parseInt(monthCombo.getValue()))
                && (Integer.parseInt(dayCombo2.getValue()) < Integer
                    .parseInt(dayCombo.getValue()))) {
              errorLabel.setTextFill(Color.web("990000"));
              ...
              // Sets graph name
              chartTitle.setText(
                  "Report for all farms during" + monthCombo.getValue() + "/" + dayCombo.getValue()
                      + "/" + yearCombo.getValue() + " to " + monthCombo2.getValue() + "/"
    ...
    HBox editDataInput = new HBox(10);

    // Makes drop down boxes for editing data
    TextField bottomFarmIDText = new TextField();
    bottomFarmIDText.setPrefWidth(75);
    TextField bottomYearText = new TextField();
    bottomYearText.setPrefWidth(50);
    ComboBox<Integer> bottomMonthCombo = new ComboBox<Integer>();
    bottomMonthCombo.setPromptText("Month");
    for (int i = 1; i <= 12; i++)
      bottomMonthCombo.getItems().add(i);

    ComboBox<Integer> bottomDayCombo = new ComboBox<Integer>();
    bottomDayCombo.setPromptText("Day");
    for (int i = 1; i <= 31; i++)
      bottomDayCombo.getItems().add(i);

    TextField bottomWeightText = new TextField();
    bottomWeightText.setPrefWidth(100);
    ...
    Button editOrAdd = new Button("Edit");
    Button delete = new Button("Delete");
    Button peek = new Button("Peek");

    editDataInput.getChildren().add(editOrAdd);
    editDataInput.getChildren().add(delete);
    editDataInput.getChildren().add(peek);
    ...
    editOrAdd.setOnAction(value -> {
      if (bottomFarmIDText.getText() != null && bottomYearText.getText() != null
        ...
        h.addFarmData(bottomFarmIDText.getText(), Integer.parseInt(bottomWeightText.getText()),
            Integer.parseInt(bottomYearText.getText()), bottomMonthCombo.getValue(),
            bottomDayCombo.getValue());
        } catch (Exception e) {
          errorLabel.setTextFill(Color.web("990000"));
    ...
    peek.setOnAction(value -> {
      if (bottomFarmIDText.getText() != null && bottomYearText.getText() != null
          && bottomMonthCombo.getValue() != null && bottomDayCombo.getValue() != null) {
        try {
          bottomWeightText.setText("" + h.getFarmData(bottomFarmIDText.getText(),
              Integer.parseInt(bottomYearText.getText()), bottomMonthCombo.getValue(),
              bottomDayCombo.getValue()));
        } catch (Exception e) {
          errorLabel.setTextFill(Color.web("990000"));
        ...
        errorLabel.setTextFill(Color.web("990000"));
        errorLabel.setText("Ensure the peek options are filled out correctly");
      }
    });
    ...
    delete.setOnAction(value -> {
      if (bottomFarmIDText.getText() != null && bottomYearText.getText() != null
        ...
        // Addition
        try {
          h.addFarmData(bottomFarmIDText.getText(), 0, Integer.parseInt(bottomYearText.getText()),
              bottomMonthCombo.getValue(), bottomDayCombo.getValue());
        } catch (Exception e) {
          errorLabel.setTextFill(Color.web("990000"));
          errorLabel.setText("Ensure the edit/addition options are filled out correctly");
        }
    ...
    HBox currentEditHbox =
        new HBox(new Label("FarmID:   "), currentTextField, new Label("   "), deleteButton, result);
    ...
    printButt.setOnAction(value -> {
      PrinterJob job = PrinterJob.createPrinterJob();
      if (job != null) {
        job.showPrintDialog(primaryStage);
        PageLayout pageLayout = javafx.print.Printer.getDefaultPrinter().createPageLayout(Paper.A4,
            PageOrientation.LANDSCAPE, javafx.print.Printer.MarginType.DEFAULT);
        job.getJobSettings().setPageLayout(pageLayout);
        double scaleX = pageLayout.getPrintableWidth() / centerVbox.getBoundsInParent().getWidth();
        double scaleY =
            pageLayout.getPrintableHeight() / centerVbox.getBoundsInParent().getHeight();
        Scale scale = new Scale(scaleX, scaleY);
        centerVbox.alignmentProperty().setValue(Pos.TOP_LEFT);
        centerVbox.getTransforms().add(scale);
        centerVbox.setTranslateX(-150);
        job.printPage(pageLayout, centerVbox);
        centerVbox.setTranslateX(0);
        centerVbox.getTransforms().remove(scale);
        job.endJob();
      }
    });
